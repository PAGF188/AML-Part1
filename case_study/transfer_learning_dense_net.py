...
def approach2(train=False):
    ...
    optimizer = torch.optim.SGD(model.classifier.parameters(), lr=0.1, momentum=0.9)

    if train:
        # Train and evaluate
        model, hist = train_model(model, dataloaders_dict, criterion, optimizer, num_epochs=num_epochs)
        # Save
        torch.save(model.state_dict(), os.path.join('./models','f_densenet_tuned.pt'))
        print(hist)
    else:
        model.load_state_dict(torch.load(os.path.join('./models','f_densenet_tuned.pt')))        
        eval_model(model, dataloaders_dict['val'], criterion)
